...
base = Path(r"C:\Users\ameyk\OneDrive\Desktop\PERSISTENT INTERNSHIP\Mood Detecting Music Player\Emotion Detection Model")
examples = [
    ...
    if path.exists():
        with path.open("r", encoding="utf-8", newline="") as f:
            ...
            for row in reader:
                ...
                if text:
                    existing.add((text, emotion))
    with path.open("a", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        appended = 0
        for text, emotion in examples:
            key = (text.strip(), emotion.strip())
            if key not in existing:
                writer.writerow([text, emotion])
                existing.add(key)
                appended += 1
        print(f"updated {path} with {appended} new rows")
